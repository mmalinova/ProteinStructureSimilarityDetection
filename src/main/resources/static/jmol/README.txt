...
Note 2021.05.27: Code is still being generated for Jmol/SVN and Jmol-SwingJS/GitHub projects
                 that is nearly the same, but still allowing only for Java 6 on SVN, since that
                 has to use the older Java2Script plug-in that Zhou Renjian only wrote to accept
                 Java source-code syntax through Java 6.
...
drag-and-drop of URLs over it, has a top menu bar, can open and save URLs,
etc. It can be open from the command line (particulary, using the shell or batch
URLs described above), but if Java is properly configured in your system, it's
...
These are used inside web pages, and include:
  Applet URLs  |  Core Javascript libraries  |  Optional Javascript libraries
...
from the server. 
It is still recommended that you put all JmolApplet0*.jar URLs on your server
even if your page does not use the capabilities provided by some of the URLs,
...
directory or folder containing the set of jar URLs:
 jmolInitialize("directory-containing-jar-URLs")
for example,
 jmolInitialize(".")  
     (if jar URLs are in the same folder as the web page)
 jmolInitialize("../jmol") 
     (if jar URLs are in a parallel folder, named 'jmol')
...
page visitor for it to run, but then it will have less security restrictions for
 file access. For example, it can access URLs on any part of the user's hard
...
Typically users get a message asking if they want to accept the "certificate" or
 ...
 be used with this in mind. Other than reading URLs, Jmol does not currently
 use other capabilities of signed applets, such as accessing the system 
 clipboard or writing URLs. Use only if you know what you are doing and have
 considered the security issues.

To use this with Jmol.js, use the form:
 jmolInitialize("directory-containing-jar-URLs", true)
or
 jmolInitialize("directory-containing-jar-URLs", "JmolAppletSigned0.jar")
...
This single file is equivalent to the whole set of JmolApplet0*.jar URLs,
explained above.
...
b) identify it explicitly in jmolInitialize(), for example:
 jmolInitialize("directory-containing-jar-URLs", "JmolApplet.jar")
...
JmolAppletSigned.jar should be used with this in mind. Other than reading URLs,
Jmol does not currently utilize other capabilities of signed applets, such as 
accessing the System clipboard or writing URLs. Use only if you know what you
are doing and have considered the security issues.

To invoke JmolAppletSigned.jar from Jmol.js, use:
 jmolInitialize("directory-containing-jar-URLs", "JmolAppletSigned.jar")
...
and set of .js URLs (described next).
